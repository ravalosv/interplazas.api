...
## Descripción
Autenticación y gestión de usuarios. Algunas rutas requieren autenticación por JWT y autorización basada en `tipoUsuarioId` usando `checkRole`.

- Base: `/api/auth`
- Autenticación:
  - `POST /login`: no requiere token
  - `POST /renew-token/`: no requiere token
  - `POST /register`: requiere `Authorization: Bearer <token>` y `checkRole([1])`
  - `PUT /change-password`: requiere `Authorization: Bearer <token>` y `checkRole([1])`
...
- Autorización: `checkRole([1])` basado en `tipoUsuarioId` del JWT
- Body:
```json
{
...
## Roles y seguridad
- `POST /register` y `PUT /change-password` están protegidas por `checkJwt` y `checkRole(roles: number[])`.
- `checkRole` valida que el `tipoUsuarioId` incluido en el JWT esté dentro del arreglo `roles` permitido.
